...
var r_lo_min := 72.0          # ward reaches r~37; longest bank pokes ~31 —
var r_lo_max := 190.0         # nothing ever laps over a balustrade
var r_hi_min := 110.0
var r_hi_max := 235.0
...
func _ready() -> void:
	...
	for i in low_count:
		_bank(rng, rng.randf_range(r_lo_min, r_lo_max),
				rng.randf_range(-18.0, -5.0), rng.randf_range(2.0, 2.8))
	for i in high_count:
