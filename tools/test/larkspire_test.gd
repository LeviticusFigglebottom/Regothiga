...
func _run() -> void:
	...
	# ---- the climb: every waypoint of the 7.2 m spiral has solid floor
	var route := [
		[Vector3(0, 0, 5), 0.0, "base hall"],
		[Vector3(-6, 2.4, -6), 2.4, "NW landing"],
		[Vector3(6, 4.8, -6), 4.8, "NE landing"],
		[Vector3(6, 7.2, 6), 7.2, "ring 1 arrival (SE)"],
		[Vector3(2, 7.2, 2), 7.2, "matins platform"],
		[Vector3(-6, 9.6, 6), 9.6, "SW landing"],
		[Vector3(-6, 12.0, -6), 12.0, "NW landing high"],
		[Vector3(6, 14.4, -6), 14.4, "ring 2 arrival (NE)"],
		[Vector3(2, 14.4, -2), 14.4, "sext platform"],
		[Vector3(5, 16.8, 6), 16.8, "SE landing"],
		[Vector3(-6, 19.2, 6), 19.2, "SW landing high"],
		[Vector3(-6, 21.6, -6), 21.6, "ring 3 arrival (NW)"],
		[Vector3(2, 21.6, -2), 21.6, "vespers platform"],
		[Vector3(5, 24.0, 6), 24.0, "SE landing top"],
		[Vector3(-6, 26.4, 6), 26.4, "SW landing last"],
		[Vector3(-6, 28.8, -6), 28.8, "summit arrival (NW)"],
		[Vector3(0, 28.8, -2), 28.8, "the gated overlook"],
		[Vector3(0, 28.8, 4), 28.8, "the Larkwarden's arena"],
	]
	for wp in route:
		var y := _floor_under(wp[0], area)
		check(absf(y - float(wp[1])) < 0.35, "floor under %s (y=%.2f)" % [wp[2], y])

	# ---- walk the first story for real: three flights, two landings
	...
	check(player.global_position.y > 4.5 and player.global_position.x > 3.5,
		"walked up the north flight (y=%.2f x=%.2f)" % [player.global_position.y, player.global_position.x])
	player.sim_move = Vector3(0, 0, 1)
	await ticks(280)
	player.sim_move = Vector3.ZERO
	await ticks(8)
	check(player.global_position.y > 6.9 and player.global_position.z > 3.5,
		"walked up the east flight to ring 1 (y=%.2f z=%.2f)" % [player.global_position.y, player.global_position.z])

	# ---- walk to each birdcage platform from its ring arrival
	var legs := [
		[Vector3(6, 7.5, 6), [[Vector3(-1, 0, 0), 55], [Vector3(0, 0, -1), 55]],
			Vector3(2, 7.2, 1.5), "matins platform walk"],
		[Vector3(6, 14.7, -6), [[Vector3(0, 0, 1), 55], [Vector3(-1, 0, 0), 65]],
			Vector3(2, 14.4, -1.5), "sext platform walk"],
		[Vector3(-6, 21.9, -6), [[Vector3(1, 0, 0), 90], [Vector3(0, 0, 1), 55]],
			Vector3(2, 21.6, -1.5), "vespers platform walk"],
	]
	for leg in legs:
	...
	# ---- the office gate seals the last ascent while the offices are unsung
	player.global_position = Vector3(6, 21.9, -5)
	player.velocity = Vector3.ZERO
	await ticks(5)
	player.sim_move = Vector3(0, 0, 1)
	await ticks(60)
	player.sim_move = Vector3.ZERO
	await ticks(5)
	check(player.global_position.z < -2.3,
		"the unsung gate holds the east band (z=%.2f)" % player.global_position.z)

	# ---- once sung, the gate opens and the last three flights crest the summit
	...
	player.sim_move = Vector3.ZERO
	await ticks(8)
	check(player.global_position.y > 23.5 and player.global_position.z > 3.5,
		"the sung gate opens the ascent (y=%.2f z=%.2f)" % [player.global_position.y, player.global_position.z])
	player.global_position = Vector3(-6, 26.7, 3)
	player.velocity = Vector3.ZERO
	await ticks(5)
	player.sim_move = Vector3(0, 0, -1)
	await ticks(300)
	player.sim_move = Vector3.ZERO
	await ticks(8)
	check(player.global_position.y > 28.5 and player.global_position.z < -3.5,
		"the west flight crests the summit (y=%.2f z=%.2f)" % [player.global_position.y, player.global_position.z])
	...
	check(bp.distance_to(Vector3(0, 28.8, 4)) < 2.5 and bp.y > 18.5,
